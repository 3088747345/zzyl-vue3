<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="projectList"> <!--@selection-change="handleSelectionChange"-->
      ...
      <el-table-column label="编号" align="center" type="index" width="80" />  <!--prop = "id"-->
      <el-table-column label="名称" align="center" prop="name" width="80" />
      ...
      <el-table-column label="操作" align="center" width="200" fixed="right" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="primary" :icon="scope.row.status === 0 ? 'Check' : 'Close'" @click="handleStatus(scope.row)">{{ scope.row.status === 0 ? '启用' : '禁用' }}</el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="排序号" prop="orderNo">
              <el-input-number v-model="form.orderNo" :min="1" :max="100" controls-position="right" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="24">
            <el-form-item label="价格" prop="price">
              <el-input-number v-model="form.price" :min="1" :max="100000" :step="5" controls-position="right" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="状态" prop="status">
              <el-radio-group v-model="form.status">
                <el-radio v-for="item in nursing_project_status" :key="item.value" :label="item.label" :value="item.value"></el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
function handleStatus(row) {
  ...
  proxy.$modal.confirm(`是否确认${msg}这条护理项目？`).then(function() {
    ...
    getList()
    row.status === 0 ? proxy.$modal.msgSuccess(`${msg}成功`) : proxy.$modal.msgError(`${msg}成功`)
  }).catch(() => {})
}
